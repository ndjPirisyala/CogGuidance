...
def flow_farneback_np(a_u8, b_u8, scale=0.25):
    """Compute Farnebäck flow A->B.

    Accepts either RGB uint8 (H,W,3) or grayscale uint8 (H,W).
    If scale != 1.0, compute flow at downsampled resolution and return flow in that downsampled pixel units.
    """
    if scale != 1.0:
...
def sobel_mag_gray_torch(g):  # g: (B,1,H,W) torch
...
def to_u8_gray_torch(x_hw: torch.Tensor) -> np.ndarray:
    """Convert a torch tensor (H,W) or (1,H,W) to uint8 grayscale (H,W) for OpenCV.
    We standardize to reduce contrast/scale confounds (important for flow).
    """
    ...
    x = (x - m) / s
    x = x.clamp(-3.0, 3.0)
    x = ((x + 3.0) / 6.0 * 255.0).clamp(0, 255).byte().numpy()
    return x
    ...
    def cb_on_step_end(pipe, step_index, timestep, callback_kwargs):
        ...
        if guide and (step_index in guided_decode_steps) and (rho_t > 0.0):
            ...
            with torch.enable_grad():
                # LATENT-SPACE guidance (no VAE decode): build an energy on latent projections.
                # x_full: (B, pack_T, C_lat, H_lat, W_lat)
                ...
                # Project latents -> grayscale "image" per slice (B,K,1,H,W)
                # (RMS over channels is simple + stable.)
                p = x_win.float().pow(2).mean(dim=2, keepdim=True).sqrt()

                # Optional downsample for cheaper flow/edge ops.
                B, T, _, H, W = p.shape
                target_long = min(guide_long_side, max(H, W))  # never upsample in guidance
                ...
                for a, b in pairs:
                    ...
                    # Farnebäck flow in latent-projection space (stop-grad, CPU)
                    Pa_u8 = to_u8_gray_torch(Pa[0, 0])
                    Pb_u8 = to_u8_gray_torch(Pb[0, 0])
                    flow = flow_farneback_np(Pa_u8, Pb_u8, scale=1.0)  # A->B at (h2,w2)

                    # Edge magnitude in latent-projection space (differentiable)
                    Ea = sobel_mag_gray_torch(Pa)
                    Eb = sobel_mag_gray_torch(Pb)

                    # normalize to avoid trivial "shrink everything" solutions
                    Ea_n = Ea / (Ea.mean(dim=(2, 3), keepdim=True) + 1e-6)
                    Eb_n = Eb / (Eb.mean(dim=(2, 3), keepdim=True) + 1e-6)

                    # warp B -> A using +flow (grid_sample is differentiable w.r.t. Eb_n)
